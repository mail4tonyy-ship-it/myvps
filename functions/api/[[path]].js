...
export async function onRequest(context) {
    ...
    if (action === "egress_result" || action === "sub") return new Response("Not Found", { status: 404 });
    ...
    try {
        ...
        if (action === "vps" && isAdmin) {
            ...
            if (method === "PUT") return Response.json({ error: "服务器出口切换已在 MyVps 中移除" }, { status: 410 });
            if (method === "DELETE") { 
                const ip = new URL(request.url).searchParams.get("ip"); 
        ...
        if (action === "nodes" || action === "thirdparty") return new Response("Not Found", { status: 404 });
