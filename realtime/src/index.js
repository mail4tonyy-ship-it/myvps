...
function requestPagesOrigin(request, env) {
  const candidates = [request.headers.get("X-MyVps-Pages-Origin"), request.headers.get("Origin")].filter(Boolean);
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/health") return json({ ok: true, service: "myvps-realtime", version: 1 }, 200, cors(request, env));

    if (url.pathname === "/agent/ws") {
      if (request.headers.get("Upgrade")?.toLowerCase() !== "websocket") return json({ error: "WebSocket required" }, 426);
      const ip = url.searchParams.get("ip") || "";
      const role = url.searchParams.get("role") || "";
      if (!['core'].includes(role)) return json({ error: "Invalid role" }, 400);
      ...
      return stub.fetch(doRequest("/ws", request, { "X-MyVps-IP": ip, "X-MyVps-ROLE": role }));
    }
    ...
    if (url.pathname === "/dashboard/ticket" && request.method === "POST") {
      ...
      const response = await hub.fetch(new Request("https://hub.internal/ticket", { method: "POST", headers: { "X-MyVps-USER": "admin" } }));
    ...
    if (url.pathname === "/public/ws") {
      ...
      const hub = env.DASHBOARD_HUB.get(env.DASHBOARD_HUB.idFromName("main"));
      return hub.fetch(doRequest("/public-ws", request, { "X-MyVps-CLIENT-IP": request.headers.get("CF-Connecting-IP") || "unknown" }));
    }
    ...
    if (url.pathname === "/public-policy" && request.method === "POST") {
      ...
      const body = await request.json().catch(() => ({}));
      const hub = env.DASHBOARD_HUB.get(env.DASHBOARD_HUB.idFromName("main"));
      const response = await hub.fetch(new Request("https://hub.internal/public-policy", { method: "POST", headers: { "X-MyVps-Public": body.public === true ? "1" : "0" } }));
      return new Response(response.body, { status: response.status, headers: { ...Object.fromEntries(response.headers), ...cors(request, env) } });
    }
...
export class VpsPresence extends DurableObject {
  constructor(ctx, env) {
    ...
    this.ctx = ctx;
    this.env = env;
    this.snapshot = { ip: "", core: null, updated_at: 0 };
  ...
  async fetch(request) {
    const url = new URL(request.url);
    if (url.pathname === "/ws") {
      const ip = request.headers.get("X-MyVps-IP") || "";
      const role = request.headers.get("X-MyVps-ROLE") || "";
    ...
    if (url.pathname === "/dashboard-active" && request.method === "POST") {
      this.setDashboardActivity(request.headers.get("X-MyVps-Active") === "1", Number(request.headers.get("X-MyVps-Interval")) || IDLE_STATUS_INTERVAL, Number(request.headers.get("X-MyVps-Until")) || Date.now() + 300000);
      return json({ success: true });
    }
  ...
  async webSocketMessage(ws, message) {
    ...
    if (!['core'].includes(role) || envelope.role !== role || envelope.ip !== attachment.ip) return;
    ...
    if (messageType === "config.result") return;
    ...
    const criticalChange = !previousRoleState;
  ...
  async markDisconnected(ws) {
    ...
    if (['core'].includes(role) && this.ctx.getWebSockets(role).length === 0) {
  ...
  syncFromSockets() {
    ...
    this.snapshot.updated_at = Math.max(this.snapshot.core_last_seen || 0, this.snapshot.updated_at || 0);
  }
  ...
  async broadcast() {
    ...
    await hub.fetch(new Request("https://hub.internal/update", {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-MyVps-Presence": "1" },
...
export class DashboardHub extends DurableObject {
  ...
  async fetch(request) {
    ...
    if (url.pathname === "/ticket" && request.method === "POST") {
      ...
      await this.ctx.storage.put(`ticket:${ticket}`, { user: request.headers.get("X-MyVps-USER") || "", expires: Date.now() + 60000 });
    ...
    if (url.pathname === "/public-ws") {
      const clientIp = request.headers.get("X-MyVps-CLIENT-IP") || "unknown";
    ...
    if (url.pathname === "/public-policy" && request.method === "POST") {
      const enabled = request.headers.get("X-MyVps-Public") === "1";
      if (!enabled) {
    ...
    if (url.pathname === "/update" && request.method === "POST") {
      if (request.headers.get("X-MyVps-Presence") !== "1") return json({ error: "Forbidden" }, 403);
  ...
  async snapshot() {
    ...
    const snapshots = await Promise.all(servers.slice(0, 100).map(async row => {
      ...
      if (live?.ip && live.core) return live;
      return {
        ip,
        transport: "http",
  ...
  async setDashboardActivity() {
    ...
    await Promise.all(servers.slice(0, 100).map(async ({ ip }) => {
      ...
      if (!name) return;
      const presence = this.env.VPS_PRESENCE.get(this.env.VPS_PRESENCE.idFromName(name));
      return presence.fetch(new Request("https://presence.internal/dashboard-active", { method: "POST", headers: { "X-MyVps-Active": active ? "1" : "0", "X-MyVps-Interval": String(interval), "X-MyVps-Until": String(until) } }));
    }));
  }
